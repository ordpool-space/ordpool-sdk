...
/**
 * Wizz CAT-21 CREATE-OFFER roundtrip on regtest — Wizz is the BUYER.
 ...
 * 2. Fund Wizz's bcrt1p with a buyer funding UTXO.
 ...
 *    bcrt1q UTXO.
 * 4. Wizz builds a buy-offer PSBT against the seller's cat with Wizz's own
 *    taproot funding UTXO at input 1. Wizz signs input 1 via
 ...
 * 6. Assert via electrs: cat's 546-sat UTXO now at Wizz's buyer-receive
 ...
 * Wizz's funding input is a Taproot key-path spend (Taproot mode), matched
...
async function shot(p: Page, name: string): Promise<void> {
  await p.screenshot({
    path: path.resolve(RESULTS_DIR, `wizz-create-offer-${name}.png`),
    fullPage: true,
  }).catch(() => undefined);
}
...
/** Wait for a Wizz sign popup and approve it (single popup on this flow). */
async function approveSignPopup(ctx: BrowserContext, knownPages: Set<Page>, tag: string): Promise<void> {
  const approval = await waitForApprovalPopup({
    context: ctx,
    knownPages,
    timeoutMs: 120_000,
    // Anchored on the real control: Wizz renders Sign as an Ant Design
    // <button>, measured in a matrix run (ant-btn ant-btn-primary, no testid).
    // Role+name matches the button itself, not a wrapper.
    isApproval: approvalGate({
      url: /notification\.html#\/approval/,
...
test('build + sign a CAT-21 buy-offer on regtest via Wizz (Taproot mode, BUYER): seller raw-key mints, Wizz signs buyer input, seller signs input 0', async () => {
  ...
  // Mainnet address Wizz matches `toSignInputs` against for the offer
  ...
  // eslint-disable-next-line no-console
  console.log(`[wizz-create-offer] buyer funded utxo ${walletFundUtxo.txid}:${walletFundUtxo.vout} (${walletFundUtxo.value} sats)`);
  ...
  // eslint-disable-next-line no-console
  console.log(`[wizz-create-offer] seller owns cat ${mintTxid}:0 at ${sellerAddress}`);

  // ── Wizz builds + signs buyer-side of the offer (1 popup) ──
  // buyerReceive / buyerChange are the wallet's regtest bcrt1p (cats land
  // at ordinals); the buyer funding input is Wizz's taproot UTXO, matched
  // by Wizz's mainnet bc1p account via `paymentAddress`.
  ...
  // eslint-disable-next-line no-console
  console.log(`[wizz-create-offer] buyer (Wizz) signed input 1; PSBT is ${buyerSignedPsbtBytes.byteLength} bytes`);
  ...
  // eslint-disable-next-line no-console
  console.log(`[wizz-create-offer] offer-acceptance broadcast txid = ${acceptTxid}`);
  ...
  // Non-witness-tamper guard: neither Wizz (buyer) nor the seller may
  ...
  // ── Assert via ELECTRS: cat's 546-sat UTXO now at Wizz's buyer-receive
  // ── (bcrt1p) output 0 ──
  ...
  // eslint-disable-next-line no-console
  console.log(`[wizz-create-offer] cat now at ${walletTaproot} (${boughtCat.txid}:${boughtCat.vout})`);
