 ...
 *
 * - Onboard Cat21 Wallet with the BIP-39 test seed via its UI
 *   (testid-driven, same flow as cat21wallet-onboard.spec.ts)
 * - Open the SDK harness page (http://localhost:4500/) which
 *   imports the real `cat21walletConnector`
 * - Drive `window.ordpoolSdkHarness.connectCat21Wallet()` — this
 *   calls `Cat21Provider.request('getAddresses')` inside the
 *   content script
 ...
 *
 * Cat21 Wallet is the model wallet for the "WE finalize, WE broadcast"
...
test.beforeAll(async () => {
  if (!fs.existsSync(path.join(EXT_PATH, 'manifest.json'))) {
    throw new Error(`Cat21 Wallet extension not unpacked at ${EXT_PATH}. This is a missing prerequisite, not a test failure: run e2e/playwright/playwright-bootstrap.sh cat21wallet.`);
  }
...
test('cat21walletConnector.connect via the harness page returns the BIP-84 / BIP-86 mainnet addresses for the test seed', async () => {
  ...
  // Cat21Provider.request('getAddresses') opens a connection-
  ...
  const resultPromise = harness.evaluate(() => window.ordpoolSdkHarness.connectCat21Wallet());

  // Cat21 Wallet's approval surface is identified by the stable testid
  ...
  console.log(`[cat21wallet:sdk-handshake] payment  = ${info.paymentAddress}`);
  // eslint-disable-next-line no-console
  console.log(`[cat21wallet:sdk-handshake] ordinals = ${info.ordinalsAddress}`);
  ...
  // Cat21 Wallet v6.x returns it in compressed form (66 hex); the SDK's
  // parseCat21WalletAddressResponse normalises via toXOnlyPubkeyHex.
