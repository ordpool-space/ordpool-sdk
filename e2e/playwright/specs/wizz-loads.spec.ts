...
/**
 * Iteration 1 of the Wizz E2E pipeline: prove that we can load
 ...
 *
 * Entry point per manifest: `temp.html`.
 */
...
test.beforeAll(async () => {
  if (!fs.existsSync(path.join(EXT_PATH, 'manifest.json'))) {
    throw new Error(
      ...
      `Run e2e/playwright/playwright-bootstrap.sh wizz first.`,
    );
  }
  const manifest = JSON.parse(fs.readFileSync(path.join(EXT_PATH, 'manifest.json'), 'utf8'));
  manifestVersion = manifest.version;
  console.log(`[wizz] loading extension v${manifestVersion} from ${EXT_PATH}`);
  ...
  console.log(`[wizz] extension id = ${extensionId}`);
});
...
test('Wizz loads in Chromium with a service worker registered; navigates to its entry point with non-empty body text', async () => {
  expect(extensionId).toMatch(/^[a-p]{32}$/);

  const startupPages = context.pages();
  console.log(`[wizz] startup pages: ${startupPages.map(p => p.url()).join(', ')}`);
  for (const [i, p] of startupPages.entries()) {
    try {
      await p.screenshot({
        path: path.resolve(__dirname, `../../../test-results/wizz-startup-page-${i}.png`),
        fullPage: true,
      });
    } catch (e) {
      console.log(`[wizz] couldn't screenshot startup page ${i}: ${(e as Error).message}`);
    }
  }
  ...
  console.log(`[wizz] navigated URL = ${finalUrl}`);
  console.log(`[wizz] page title    = ${title}`);

  await page.screenshot({
    path: path.resolve(__dirname, `../../../test-results/wizz-index.png`),
    fullPage: true,
  });

  const bodyHtml = await page.evaluate(() => document.body.innerHTML.slice(0, 4000));
  fs.writeFileSync(
    path.resolve(__dirname, `../../../test-results/wizz-body-snippet.html`),
    bodyHtml,
  );
