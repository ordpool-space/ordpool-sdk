...
/**
 * Iteration 2 of the Cat21 Wallet E2E pipeline: drive the onboarding
...
// Cat21 Wallet's password-strength meter (likely zxcvbn) rates the
// shared `TestPassword123!` as "Poor" and refuses to enable
// Continue. CI 26375584175 / 05-password-typed.png shows the rule
// at work. Use a longer high-entropy passphrase for Cat21 Wallet only;
...
async function shot(page: Page, name: string): Promise<void> {
  await page.screenshot({
    path: path.resolve(RESULTS_DIR, `cat21wallet-onboard-${name}.png`),
    fullPage: true,
  }).catch(() => undefined);
}
...
async function dumpHtml(page: Page, name: string): Promise<void> {
  try {
    ...
    fs.writeFileSync(path.resolve(RESULTS_DIR, `cat21wallet-onboard-${name}.html`), html);
  } catch {
    // ignore
  }
}
...
test('restores a wallet from the BIP-39 test seed and reaches a screen mentioning send/receive/balance/account/bitcoin', async () => {
  ...
  console.log(`[cat21wallet:onboard] dashboard rendered — wallet committed.`);
});
