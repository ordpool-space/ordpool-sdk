 ...
 */
async function installWizzOfflineRoutes(context) {
    ...
    await context.route('**/ep.wizz.cash/**', (route) => {
        ...
        return route.fulfill(okJson(u.includes('listscripthash')
    ...
    await context.route('**/mempool.space/api/blocks/tip/height**', route => route.fulfill({ status: 200, contentType: 'text/plain', headers: { 'access-control-allow-origin': '*', 'cache-control': 'no-store' }, body: '900000' }));
