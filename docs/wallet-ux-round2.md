...
| 3 | **Number formatting across the ecosystem**, now two divergences rather than one. Sats: cat21-wallet renders `1 234 567 sats`, ordpool.space renders `299,046 sat`. Fee decimals: cat21.space follows the browser locale (`2,5` on a German machine), ordpool.space is fixed dots (`2.5`). Each site is internally consistent, which was the actual defect and is fixed everywhere; across sites they disagree. One decision covers both, and if it is wanted it wants ONE shared formatter, which puts it in the SDK. Detail below.|
| 3b | **Sats formatting across the ecosystem.** cat21-wallet renders `1 234 567 sats`, ordpool.space renders `299,046 sat`. The wallet's was a correctness fix (locale-dependent output flipped meaning per machine); ordpool's is fixed English and cannot flip. So this is consistency, not correctness. If it is wanted, it wants ONE shared formatter, which puts it in the SDK. |
