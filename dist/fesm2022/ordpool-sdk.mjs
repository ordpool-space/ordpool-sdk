 ...
 */
function simulateMintTransaction(walletType, recipientAddress, paymentOutput, paymentAddress, paymentPublicKey, transactionFee, network) {
    const { dummyPrivateKey } = getDummyKeypair(toScureNetwork(network));
    const result = createTransaction(walletType, recipientAddress, paymentOutput, paymentPublicKey, paymentAddress, transactionFee, true, network);
...
class Cat21Service {
    ...
    simulateTransaction(walletType, recipientAddress, paymentOutput, paymentAddress, paymentPublicKey, transactionFee) {
        ...
        return simulateMintTransaction(walletType, recipientAddress, paymentOutput, paymentAddress, paymentPublicKey, transactionFee, this.network);
    }
    /**
 ...
 */
function selectCardinalUtxo(candidates, targetValueSats, preferUnder) {
    ...
    const ordered = [...candidates].sort((a, b) => a.txid === b.txid ? a.vout - b.vout : a.txid < b.txid ? -1 : 1);
 ...
 */
function estimateTaprootVbytes(numInputs, outputScriptLengths) {
 ...
 */
function liftRecommendationByOutpoint(rec, source) {
    const byOutpoint = new Map(source.map((u) => [`${u.txid}:${u.vout}`, u]));
    const lift = (c) => {
    ...
    return {
        ...
        candidates: rec.candidates.map(lift).filter((x) => x !== null),
    };
}
 ...
 */
function buildTransfer(params, funding, feeSats, isSimulation) {
...
async function planTransfer(params, ports) {
    const empty = recommendFunding([], 0);
    if (!params.feeRatePerVbyte || params.feeRatePerVbyte <= 0) {
        return { status: 'insufficient', recommendation: empty, pick: null, built: null, vsize: null, buildFeeSats: null };
    }
 ...
 */
async function simulateTransfer(params, ports) {
...
function validateCat21BuyOfferPsbt(args) {
    ...
    if (sellerInput.witnessUtxo?.amount == null) {
        return fail('wrong-seller-input-value', 'seller input 0 has no witnessUtxo (legacy P2PKH): its value is not committed by the sighash and cannot be safely valued');
    }
 ...
 */
function buildOffer(params, funding, feeSats, isSimulation) {
...
async function planOffer(params, ports) {
    const empty = recommendFunding([], 0);
    ...
    const feeBudget = (coinValue) => coinValue - params.priceSats - params.targetCat.value;
    const offerVsize = (built) => computePsbtVsize({
        psbt: built.psbt,
 ...
 */
async function simulateCreateOffer(params, ports) {
 ...
 */
async function createOffer(params, ports) {
    const plan = await planOffer(params, ports);
 ...
 */
async function acceptOffer(params, ports) {
    ...
    let channel = 'mempool';
    const { txId } = await firstValueFrom(signer.signOfferAccept({
        psbtBytes: params.offerPsbt,
        ordinalsAddress: params.ordinalsAddress,
        ordinalsPublicKey: params.ordinalsPublicKey,
        network: params.network,
        broadcast: (txHex) => from(ports.broadcast.broadcast(txHex).then((r) => {
            channel = r.channel;
            return r.txid;
        })),
        promptForSignedPsbt: prompt ? (unsigned) => from(prompt(unsigned)) : undefined,
 ...
 */
function evaluateAgentPolicyCaps(policy, action) {
